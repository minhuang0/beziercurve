function Beziercurve(args) {
    ...
    var operation = document.createElement('div'),
        repeal = document.createElement('button'),
        exportDataBtn = document.createElement('button'),
        exportTextArea = document.createElement('textArea'),
        importDataBtn = document.createElement('button'),
        importTextArea = document.createElement('textArea');
    repeal.innerText = "删除最后添加的线条";
    exportDataBtn.innerText = "导出数据";
    importDataBtn.innerText = "导入数据";
    ...
    exportDataBtn.onclick = function() {
        exportTextArea.value = JSON.stringify(self.exportData());
        return exportTextArea.value;
    };

    importDataBtn.onclick = function() {
        console.info(new Array(importTextArea.value));
    };

    operation.appendChild(exportDataBtn);
    operation.appendChild(exportTextArea);
    operation.appendChild(importDataBtn);
    operation.appendChild(importTextArea);
    ...
    function pointIsToExist(args) {
        ...
        allPoints.forEach(function(point, idx) {
            if (y > point.top - point.r && y < point.top + point.r && x > point.left - point.r && x < point.left + point.r) {
                pointIsExist = true;
                isToExistCallBack && isToExistCallBack(allPoints, idx);
            }
        });
        return pointIsExist;
    };
